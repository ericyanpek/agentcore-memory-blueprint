...
这里的记忆是**带权威等级的受治理资产**，而不是一个更聪明的向量库。由此导出四条属性，
每一条都已在真实部署上端到端验证 —— 见[实验报告](docs/实验报告.md)（14 项检查）与
[桌面客户端集成设计](docs/桌面客户端集成设计.md)（8 + 17 项检查）。

1. **隔离由 AWS 强制，而非由应用代码保证。** 单个共享 IAM role，配合 Cognito Identity
   Pool 从已验签 `sub` claim 派生的 session tag，使每个桌面客户端拥有各自的边界。已做
   镜像对照测试：同一个 role 允许 Alice 访问 Alice、拒绝其访问 Bob，而 Bob 登录后权限
   完全反转。新增工程师无需在 AWS 侧做任何改动。
2. **共享写入路径是关闭的。** 任何智能体与桌面客户端都不持有
   `BatchCreateMemoryRecords`。团队知识只能被**提案**；自动政策闸门在任何人看到它之前
   先行运行，且只有发布器 Lambda 执行写入。这是事前闸门，不是事后策展。
3. **已批准文本逐字发布。** 没有第二个抽取模型改写已审核的陈述，因此审核员批准的内容
   与入库内容逐字节一致，并记录批准人身份与证据引用。
4. **检索优先级是确定性的。** 实时数据高于文档，文档高于已审核的团队记忆，团队记忆
   高于个人偏好。记忆永远不能覆盖当前数据。

这些选择的理由，以及支持与反驳各项的外部证据：
**[设计取舍依据](docs/设计取舍依据.md)**。已知差距与后续计划：
**[下一步演进](docs/下一步演进.md)**。
